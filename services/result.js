...
const createResult = async (userBody) => {
    ...
    if (existingResult) {
        throw new ApiError(httpStatus.CONFLICT, 'Result already exists for this user and examination');
    }

    // Check if the user with the provided ID exists
    const existingUser = await User.findById(user_id);
    if (!existingUser) {
        throw new ApiError(httpStatus.NOT_FOUND, 'User with the provided ID does not exist');
    }
    ...
    if (!existingExamination) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Examination with the provided ID does not exist');
    }
    ...
    try {
        ...
        console.error(error);
        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Error creating the result');
    }
};
...
const getResultByStudentId = async (studentId) => {
    try {
        ...
        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Error retrieving results');
    }
};
