...
const generateQuestions = async (subjectName) => {
    try {
        ...
        if (!existingSubject) {
            throw new ApiError(httpStatus.NOT_FOUND, 'Subject with the provided name does not exist');
        }
        ...
        const allQuestions = [...easyQuestions, ...mediumQuestions, ...hardQuestions, ...randomQuestion];
        const shuffledQuestions = (allQuestions);

        return shuffledQuestions;
    } catch (error) {
        console.error(error);
        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Error generating random questions for exam');
    }
};
